...
function applyDateFilter(
  ...
  return {
    totalSpent: Math.round(
      data.totalSpent * multiplier
    ),
    categories: data.categories.map((cat) => ({
      ...cat,
      amount: Math.round(
        cat.amount * multiplier
      ),
    })),
  };
}
...
export default function DashboardScreen() {
  ...
  const filterAnim = useRef(
    new Animated.Value(
      DATE_RANGES.indexOf('This Month')
    )
  ).current;
      ...
      {data && filteredData && (
        ...
        >
          ...
          <FadeSlide delay={200}>
            ...
            <SpendingBreakdown
              categories={filteredData.categories.map(
                (cat) => ({
                  ...cat,
                  icon:
                    cat.icon ||
